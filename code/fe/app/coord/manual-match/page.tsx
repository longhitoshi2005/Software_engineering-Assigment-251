...
import { STUDENTS, TUTORS, type Student, type Tutor } from '@/src/lib/mocks';
import ComboBox from '@/src/components/coord/ComboBox';
import type { RankedTutor } from '@/src/types/matching';
...
const ManualMatch: React.FC = () => {
  ...
  const handleAssign = () => {
    ...
    fetch('/api/assignments', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
      .then(async (res) => {
      ...
      <main className="max-w-5xl mx-auto mt-4 grid grid-cols-1 lg:grid-cols-[1.4fr,1fr] gap-6">
            ...
            <ComboBox
              ...
              onChangeText={(v) => {
                ...
                const matched = TUTORS.find((t) => {
                  const name = (t.name ?? "");
                  return `${t.id} — ${name}` === v || t.id.toLowerCase() === v.toLowerCase() || name.toLowerCase() === v.toLowerCase();
                });
                if (matched) setForm((p) => ({ ...p, tutorId: matched.id }));
                else setForm((p) => ({ ...p, tutorId: "" }));
              }}
              onSelect={(t) => {
                setForm((p) => ({ ...p, tutorId: (t as any).id }));
        ...
        <aside className="bg-white rounded-lg border border-black/5 p-4 space-y-4">
          ...
          <div className="bg-soft-white-blue/40 border border-black/5 rounded-md p-3 space-y-2">
            ...
            {student ? (
              <>
                <p className="text-sm font-medium">{student.name} ({student.id})</p>
