...
import { QuizSource, QuizDifficulty } from "@/types";
import type { QuizStatus, QuizItem } from "@/types";

export default function AIQuizGeneratorPage() {
  const [source, setSource] = useState<QuizSource>(QuizSource.TRUSTED);
  const [topic, setTopic] = useState("CO1001 – Recursion / Pointer Debugging");
  const [difficulty, setDifficulty] = useState<QuizDifficulty>(QuizDifficulty.INTRO);
        ...
        <div className="bg-white rounded-xl border border-black/5 p-5 shadow-sm space-y-6">
          ...
          <div>
            ...
            <div className="grid md:grid-cols-2 gap-3">
              <button
                type="button"
                onClick={() => setSource(QuizSource.TRUSTED)}
              ...
              <button
                type="button"
                onClick={() => setSource(QuizSource.AI)}
                className={`p-4 rounded-lg border-2 text-left transition ${
                  source === QuizSource.AI
                    ? "border-light-heavy-blue bg-blue-50"
                    : "border-soft-white-blue bg-white hover:border-light-light-blue"
                ...
                {lastGenerated.map((q, idx) => (
                  ...
                  >
                    <div className="flex items-start gap-2 mb-3">
                      ...
                      >
                      {q.source === QuizSource.TRUSTED ? "Bank" : "AI"}
                      </span>
                    </div>
