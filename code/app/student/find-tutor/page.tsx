...
export default function FindTutorPage() {
  ...
  const filteredTutors = useMemo(() => {
    return tutors.filter((tutor) => {
      ...
      // Expertise filter (tutor.expertise may be an array)
      if (expertiseFilter) {
        const expText = Array.isArray(tutor.expertise) ? tutor.expertise.join(' ') : String(tutor.expertise ?? '');
        if (!expText.toLowerCase().includes(expertiseFilter.toLowerCase())) return false;
      }
