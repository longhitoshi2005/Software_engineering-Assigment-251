...
export default function Page() {
  ...
  async function load() {
    const res = await fetch("/api/dept/exports", {
      headers: {
    ...
    if (!res.ok) {
      console.error("Export API error:", await res.text());
      return;
    }
  ...
  async function add(name: string) {
    ...
    if (!res.ok) {
      console.error("Add export job failed:", await res.text());
      return;
    }
    ...
    <div className="max-w-6xl mx-auto px-4 md:px-6 py-6 space-y-6">
      ...
      <section className="bg-white border border-soft-white-blue rounded-lg p-5 space-y-3">
        <div className="flex flex-wrap gap-2">
          <button
            onClick={() => add("Departmental (PDF)")}
            className="px-3 py-2 text-sm rounded border hover:bg-soft-white-blue"
