...
export default function TutorProgressLogPage() {
  ...
  // Use machine enum values for select fields (stored value), labels remain human-friendly
  const [understanding, setUnderstanding] = useState("EXCELLENT");
  ...
  const handleSaveDraft = async () => {
    const payload = {
      ...
      understanding,
      // summary and next plan would be pulled from form fields in a fuller implementation
      status: "draft",
    };
    try {
      await fetch("/api/tutor/progress-log", {
        method: "POST",
      ...
      await swalError?.("Save failed", "Could not save progress draft. Please try again.");
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await swalError?.("Submit failed", "Could not submit progress log. Please try again.");
    }
  };
          ...
          <form className="bg-white rounded-lg border border-black/5 p-4 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1">
                ...
                >
                  {/* Value gửi đi là ENUM (viết hoa), Label hiển thị là Text */}
                  <option value="EXCELLENT">Excellent - fully grasped</option>
                  <option value="GOOD">Good - can apply</option>
                  <option value="FAIR">Fair - needs more practice</option>
                  <option value="POOR">Poor - struggled</option>
                </select>
