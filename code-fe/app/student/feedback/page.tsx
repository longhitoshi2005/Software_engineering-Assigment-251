...
import { CLARITY_CHOICES } from "@/lib/enums";
...
function SubmitFeedbackContent() {
  ...
  // Use machine enum values (upper-case) as the stored value; UI shows human labels
  const [clarity, setClarity] = useState("VERY_CLEAR");
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      await swalError?.("Save failed", "Could not save draft. Please try again.");
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await swalError?.("Submit failed", "Could not submit feedback. Please try again.");
    }
  };
